<script type="text/javascript">
    $(document).ready(function() 
    {
        $("#image_input").uploadify({
            ...
            'script'         : '<?php echo base_url(); ?>assets/PHPforAJAX/uploadify_advertisement.php',
            'cancelImg'      : '<?php echo base_url(); ?>assets/images/admin/cross.gif',
            'folder'         : '<?php echo base_url(); ?>assets/uploads/advertisement_images',
            'queueID'        : 'fileQueue',
            'auto'	         : false,
            'multi'	         : true,
            ...
            'onComplete': function(event, queueID, fileObj, response, data) 
            {
                ...
                $('#images_uploaded').append('<div id="uploaded_'+response+'"><a target="_blank" href="<?php echo base_url(); ?>assets/uploads/advertisement_images/'+response+'"><img alt="" border="0" src="<?php echo base_url(); ?>assets/uploads/advertisement_images/'+response+'" width="128" /></a>&nbsp;position&nbsp;<input type="text" name="image_position[]" size="5" maxlength="2" /><img class="img_del" id="'+response+'" src="<?php echo base_url(); ?>assets/images/admin/cross.gif" alt="" border="0" style="padding-right:5px; cursor: pointer;" /><input type="hidden" name="image_names[]" value="'+response+'" /></div><br />');
                $.post("<?php echo base_url(); ?>advertisement/process_uploaded_image", { user_file: response }, 
                function(data) 
                { 
                    $('#images').val(data);
                });
        ...
        $(".img_del").live('click', function() 
        {
            ...
            $.post("<?php echo base_url(); ?>advertisement/delete_image", { img: image_name }, 
            function(data) 
            { 
                if( data != 'failure' )
                {
