...
/**
 * List and manage stock of products. 
 * 
...
class Product_stock extends CI_Controller
{
    ...
    /**
     * Restricted page to list item-stock info. 
    ...
    public function index()
    {
        ...
        $stock_record          = $this->common_model->query_all_data('boutique_product_stocks');
        $num_of_rows           = count($stock_record);
        $data['num_of_stocks'] = $num_of_rows;
        if ($num_of_rows > 0)
        {
            $data['table_title'] = 'Item Stock List';
            ...
            $list_end                 = 10;	
            $data['all_stock']       = $this->product_stock_model->get_pagewise($list_start, $list_end);
            $data['stock_result']     = $data['all_stock']['stock_afterPg']; 
            $data['stock_rownumbers'] = $data['all_stock']['stock_rows'];

            $list_config['base_url']   = base_url().'product_stock/index/';
            $list_config['uri_segment']= '3';
            $list_config['total_rows'] = $data['stock_rownumbers'];
            $list_config['per_page']   = '10';		

            $this->pagination->initialize($list_config);
            $this->pagination->create_links();

            $data['stock_paging'] = $this->pagination->create_links();
            //ends paging configaration

            $this->template->write_view('main_content', 'product_stock/list', $data, TRUE);
            $this->template->render();
        }//end if
        else
        {
            redirect('product_stock/manage', 'refresh');
        }
    }//end stock listing
    ...
    public function manage($id = NULL)
    {
        ...
        {
            /* Show the form */
            $this->template->write_view('main_content', 'product_stock/manage', $data, TRUE);
        } //end main if
        else
        {
            ...
            if ($id == NULL)
            {
                ...
                $user_data['quantity']     = sha1($this->input->post('quantity'));
    ...
    public function delete()
    {
       ...
       /* ends to delete single stock */ 
        ...
        redirect(base_url().'product_stock', 'refresh');
    }

}

/* End of file product_stock.php */
/* Location: ./application/controllers/product_stock.php */